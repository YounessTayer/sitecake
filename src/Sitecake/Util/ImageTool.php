...
class ImageTool
{
	...
	static protected $mimeExtensions = [
		'jpg' => 'image/jpg',
		'jpg' => 'image/jpeg',
		'jpg' => 'image/pjpeg',
		'gif' => 'image/gif',
	...
	public function image($uri, $x = '0%', $y = '0%', $w = '100%', $h = '100%')
	{
		...
		else
		{
			...
			$this->transform($imageName['name'], $imageName['extension'],
				WideImage::loadFromString($this->fs->read($uri)), $x, $y, $w, $h);
		}
	}

	protected function parseImgPathSet($id)
	{
		//$pathPattern = $this->fs->listPatternPaths($pathPattern);
	}
	...
	protected function imageName($uri)
	{
		...
		return ['name' => $info['filename'], 'extension' => $info['extension']];
	}
