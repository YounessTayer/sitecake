...
class ErrorHandler
{
    ...
    protected static $_suppressed = self::SUPPRESS_NONE;
    ...
    public static function register($config = [])
    {
        ...
        set_exception_handler('\\Sitecake\\Error\\ErrorHandler::handleException');
    ...
    public static function handleException(\Exception $exception, $return = false)
    {
        ...
        if($return)
        {
            return self::_displayError($data, true);
        }

        self::_displayError($data, $return);
    }
     ...
     * @return string|void
     */
    protected static function _displayError($error, $return = false)
    {
        if(self::$_suppressed != self::SUPPRESS_NONE && !$return)
        {
            if(self::$_suppressed == self::SUPPRESS_NEXT)
            {
                self::$_suppressed = self::SUPPRESS_NONE;
            }
     ...
     * @return bool
     */
    protected static function _logError($level, $data = [])
    {
        if($level instanceof \Exception)
        {
            return Log::write('error', self::_getMessage($level));
        }
